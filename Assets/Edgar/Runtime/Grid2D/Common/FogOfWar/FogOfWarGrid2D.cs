...
namespace Edgar.Unity
{
    ...
    {
        /// <summary>
        /// Whether to use a mesh instead of an image effect shader (experimental).
        /// </summary>
        public bool UseMesh = false;
        
        /// <summary>
        /// If UseMesh is enabled, we need a mesh render to which the fog will be applied.
        /// </summary>
        ...
        public void Setup(GameObject generatedLevelRoot, Transform waveOrigin = null)
        {
            ...
            if (UseMesh)
            {
                if (!MeshRenderer)
                {
                    throw new ArgumentNullException(nameof(MeshRenderer), "When UseMesh is enabled, you must provide a MeshRenderer to which the fog will be applied.");
                }
        ...
        private void Update()
        {
            ...
            if (UseMesh && MeshRenderer)
            {
                MeshRenderer.material = GetMaterial(GetComponent<Camera>(), "Edgar/FogOfWarMesh"); 
            }
        }
        ...
        private Dictionary<Vector2Int, TileInfo> GetTilesToReveal(List<RoomInstanceGrid2D> roomsToReveal)
        {
            ...
            // WARNING: this logic is duplicated below
        ...
        // WARNING: this function is duplicated above
        private IEnumerable<RoomInstanceGrid2D> GetCorridorsToReveal(List<RoomInstanceGrid2D> roomsToReveal)
        {
            if (RevealCorridors && RevealCorridorsTiles > 0)
            {
                foreach (var room in roomsToReveal)
        ...
        {
            var corridorsToReveal = GetCorridorsToReveal(rooms).Distinct().ToList();
            var callbacks = rooms
                .Concat(corridorsToReveal)
                .Select(x => x.RoomTemplateInstance.GetComponentInChildren<FogOfWarCallbackGrid2D>())
                .Where(x => x)
                .ToList();

            foreach (var callback in callbacks)
            {
                callback.OnRoomRevealing();
            }
        ...
        public Material GetMaterial(Camera camera, string shaderName = "Edgar/FogOfWar")
        {
            if (material == null)
            {
                ...
                if (shader == null)
                {
                    throw new ArgumentException(
                        $"Shader with name '{shaderName}' was not found. Make sure it's included in the build.");
                }
            ...
            if (QualitySettings.activeColorSpace == ColorSpace.Linear && !UseMesh)
            {
        ...
        {
            var go = new GameObject("Fog Of War Mesh");
            
            var meshFilter = go.AddComponent<MeshFilter>();
            var vertices = new Vector3[4]
            {
                new Vector3(0 -0.5f, 0-0.5f, 0),
                new Vector3(1-0.5f, 0-0.5f, 0),
                new Vector3(0-0.5f, 1-0.5f, 0),
                new Vector3(1-0.5f, 1-0.5f, 0)
            };
